...
function statusFromVotes(confirmCount = 0, denyCount = 0) {
  if (confirmCount >= 7 && confirmCount > denyCount) return 'confirmed';
  if (denyCount >= 7 && denyCount > confirmCount) return 'rejected';
  if (confirmCount >= 3 && confirmCount > denyCount) return 'confirmed';
  if (denyCount >= 3 && denyCount >= confirmCount) return 'disputed';
  return 'pending';
}
...
function mapReport(row, photoResolver = (photos) => photos) {
  ...
  return {
    ...
    voteTally: {
      confirm: row.vote_confirm_count ?? row.voteConfirmCount ?? 0,
      deny: row.vote_deny_count ?? row.voteDenyCount ?? 0,
    },
...
function toDbPayload(input = {}) {
  ...
  return {
    ...
    notes,
    status: 'pending',
    vote_confirm_count: 0,
    vote_deny_count: 0,
    has_photos: false,
    photos: [],
    localized_by_name: null,
    localized_by_phone: null,
    localized_note: null,
    localized_at: null,
    localized_reported_by: null,
  };
}
